...
async function loadHeader() {
  try {
    ...
    let headerUrl = "../partials/store-header.html";
    if (window.location.pathname.includes("/products/")) {
        headerUrl = "../../partials/store-header.html";
    }

    const response = await fetch(headerUrl + '?v=' + new Date().getTime());
    if (!response.ok)
    ...
    if (window.location.pathname.includes('/products/')) {
        headerHtml = headerHtml.replace(/href="assets\/css\/store\.css"/g, 'href="../../assets/css/store.css"');
        headerHtml = headerHtml.replace(/src="assets\/js\/store\.js"/g, 'src="../../assets/js/store.js"');
    }
        ...
        <div class="modal-content profile-modal">
            ...
            <div class="modal-body">
                <div class="profile-info">
                    <div class="profile-avatar">${userAuth.currentUser.name.charAt(0).toUpperCase()}</div>
